...
#define SYSTEM_MEMORY_MB     128 // representing total system memory size
...
struct PageDirectoryEntryFlag {
    uint8_t present_bit        : 1; //  indicate whether a page is present in memory or not.
    // : 1 means that only 1 bit will be allocated for present_bit.
    uint8_t write_bit          : 1; // indicates whether the page is writable or read-only.
    uint8_t use_pagesize_4_mb  : 1; // indicates whether the page uses a 4 MB page size.
